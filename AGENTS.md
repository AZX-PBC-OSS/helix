...
**Current status: M2 — Edge serving.** The portal owns a Postgres schema (apps, versions, audit) via Prisma 7 and exposes a versioned REST API: create/list/get apps, upload a bundle (zip validation + static-files-only check + CSP courtesy lint + store to Blob), list versions, promote/rollback the live pointer, and archive/unarchive apps. The `azx` CLI (`packages/cli`) drives deploys. The edge serves deployed apps on `<slug>.localtest.me`: host-keyed routing, an in-memory registry projection refreshed via Postgres LISTEN/NOTIFY (the portal owns the trigger migration), asset streaming from Blob over undici with hand-rolled SharedKey signing (no Azure SDK in the edge), baseline §4.4 CSP on HTML, SPA fallback, 404 for unknown/no-live apps and 410 + `Clear-Site-Data` for archived ones — see `apps/edge/README.md`. Serving needs `EDGE_DEV_ALLOW_UNAUTHENTICATED=true` (dev bypass, set by the dev container; fail-closed until M3 auth). Portal auth is a dev-token stub on mutating routes (real OIDC/Entra is M3); sessions and the gateway are M3–M4 (see project plan §4).
...
| `pnpm dev:edge`                          | Run azx-edge (`:8080` — serves apps at `http://<slug>.localtest.me:8080`, platform `GET /health`)                                                                                 |
