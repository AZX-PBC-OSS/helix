...
- **Before enabling the surface in a real (non-local) deployment:** a short-window throttle on the dev-gateway itself, a distinct dev LLM budget (the vendor key is env-agnostic — design §11), and host/TLS wiring for `dev-api.<base>`. _(The `EDGE_TRUST_PROXY` half of that rider is closed — the trusted-ingress address is passed to the dev-gateway container, and since 2026-09-03 it is the right address: `100.64.0.0/10`, the RFC 6598 range ACA's ingress pods come from, not the apps subnet the earlier default named. A throttle added here keys on the real client IP. See ADR-0011's 2026-09 amendment.)_
