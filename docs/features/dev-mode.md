...
- **Before enabling the surface in a real (non-local) deployment:** a short-window throttle on the dev-gateway (issue #13 / `EDGE_TRUST_PROXY`), a distinct dev LLM budget (the vendor key is env-agnostic — design §11), and host/TLS wiring for `dev-api.<base>`.
