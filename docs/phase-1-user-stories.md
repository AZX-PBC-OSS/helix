...
|---|---|---|---|
| Upload a React bundle / HTML file and get a secure hosted URL | P0 | ✅ | Zip-upload deploy (CLI `helix deploy` or the portal drop-zone) → immutable version → `<slug>.<base>` behind the edge. [registry-and-deploys](./features/registry-and-deploys.md), [edge-serving](./features/edge-serving.md) |
