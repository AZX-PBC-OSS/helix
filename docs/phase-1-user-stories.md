...
| View usage metrics (sessions, errors, latency) | P1 | ⚠️ | Per-app and platform **usage dashboards are real** — tokens, request counts, outcome (ok/error/quota), by model/capability/user. *Gap:* no **sessions**, **latency**, or detailed **error** dimensions; the `gateway_calls` ledger doesn't capture them yet. [llm-gateway](./features/llm-gateway.md), [portal-web](./features/portal-web.md) |
...
| Embed a client org's licensed API tokens (e.g. CBRE's own contracts) | P0 | ✅ | **Global** secrets + per-app **grants**: store the client's token once, grant the apps that may spend it; one secret backs many connections, so rotation is one write. [secrets-and-connections](./features/secrets-and-connections.md) |
...
| Alert when usage/spend thresholds are exceeded | P1 | ⬜ | Per-app daily **budgets** exist and block-new once hit (runaway is *capped*), but there is no **alerting**, and spend is tracked in tokens, not dollars. [llm-gateway](./features/llm-gateway.md) |
...
what they would land on. The Heatwave app in particular leans on two areas still in motion — a
**geocoding/weather** third-party endpoint (today via fetch-proxy + a connection, not a
pre-provisioned catalog entry) and richer **usage metrics** — worth tracking as the pilot scopes.
...
**governance depth** (team-scoped policy, tamper-evident audit). The clearest unbuilt asks are
**alerting** and **warehouse export** (P1/P2).
