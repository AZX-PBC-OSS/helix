...
What actually helps: **asymmetric** signing (egress holds a verify-only public key) shrinks forgeability against a compromised *egress* — the secret-holding plane, the higher-value target. Keep it as **step 3** (larger, key-mgmt/rotation), not step 1. The cheap, real wins now are the **`jti`/`aud` burn** (step 1, **#3**) and **binding method+path** (**#6**) — both independent of the key strategy.
