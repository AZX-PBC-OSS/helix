...
### Considered and rejected (2026-06-26): move the OIDC RP credential off the edge

A review proposed elevating "get the OIDC client secret / IdP token-exchange off the edge (to egress or a broker)" to P0, parallel to the Blob-key fix. **Rejected — don't file** (5-model panel + best-practice grounding):
- Holding the relying-party credential on the **auth-terminating** component is the **standard BFF / token-handler / confidential-client pattern** (IETF browser-apps draft; Auth0/Curity), not an anti-pattern.
- It does **not** generalise from the Blob key: an edge RCE can already mint sessions via `EDGE_AUTH_SECRET` + the Postgres `sessions` table **without** the IdP credential — the edge *is* the auth authority — so relocating the RP credential doesn't shrink the realistic blast radius; a broker just holds an equivalent secret. Prod uses an **IdP-revocable cert**; blast radius is RP impersonation at one IdP, not all-tenant data.
- The real lever for the edge's **operational-secret cluster** (`EDGE_AUTH_SECRET`, `HELIX_INSTRUCTION_SECRET`, the OIDC cert) is Key Vault / managed-identity + rotation — optionally consolidated behind a KMS root with short-lived derived keys (a one-pager, not a fourth container). Distinct from #15, which *is* a P0 because the Blob key is an over-privileged *platform* credential with all-tenant RW.
