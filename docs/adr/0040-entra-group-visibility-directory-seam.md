...
- **`payload.groups ?? payload.roles` is a live landmine and must be fixed first.** `verifier.ts:125` uses `??`, not a union. Today `groups` is absent, the fallback fires, and `roles` delivers `platform-admin`. The moment decision 6 adds SecurityGroup claims to the portal registration, `groups` becomes present and truthy, the fallback stops firing, and **admin gating breaks in production** — approvals and every admin page lock out every admin — triggered by an Entra-side configuration change with no code deploy and no failing test. This must become a concatenation of both claims, with a test pinning it, *before* the registration is touched. It is the single most dangerous item in this ADR and the least visible.
