...
- **ABA across delete + recreate.** `version` is a per-row counter starting at 1, so `DELETE` followed by re-creation restarts it: a stale `If-Match: "1"` would match a re-created value the writer never read — the exact lost update CAS prevents. Delete exists only on `user` scope today, so the blast radius is one person's own tabs; this becomes real the moment a shared delete verb lands, and the fix (a row-birth nonce folded into the ETag, or a key-scoped sequence) should ship with that verb, not before.
