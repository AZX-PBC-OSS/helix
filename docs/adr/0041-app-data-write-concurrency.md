...
### 7. The loser is not metered and not charged

A `412` or `428` records no `gateway_calls` row and does not count against `writesPerDay`. The write did not happen, and a contended retry loop must not consume the app's daily budget — that would turn contention into a self-inflicted quota outage.

No new abuse control is needed. The anonymous tier's per-IP limiter already runs inside `preamble` (`data-handler.ts:123`), before any store work, so a `412` spin loop is bounded before it reaches the `UPDATE`.
