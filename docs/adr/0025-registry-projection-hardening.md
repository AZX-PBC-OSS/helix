...
**Residuals:** no metrics pipeline and no alert rule yet exist to consume any of this — until one is created in Log Analytics, the degradation is only visible to a human. `consecutiveLoadFailures` counts *attempts*, not time, since NOTIFY-driven loads can inflate it faster than the reconcile interval — which is why the age rule is the authority for `error` wherever an age exists. (Egress's two pools used to bypass `createEdgePool` entirely and get no `statement_timeout`; closed 2026-09-23 — they build through egress's own `createEgressPool`, `apps/egress/src/pool.ts`, which applies the same ceiling with `EGRESS_STATEMENT_TIMEOUT_MS` to tune. They need none of the edge factory's `withPooledClient` machinery: their stores run `Pool.query()` only, and a bare `pool.connect()` is lint-banned on that plane too.)
