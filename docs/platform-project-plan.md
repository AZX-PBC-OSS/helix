...
5. **Deploy skill + preview/promote** — **partial.** Preview-by-default deploys + human promote/rollback are done (control plane + `helix` CLI), and the device-code flow exists (`helix login`/`logout`/`whoami`, XDG token cache). _Remaining:_ the `packages/deploy-skill` agent bundle (does not exist yet).
