...
The fetch-proxy and secret-backed connections **shipped as M4.5** (above) — built on the `azx-egress` mechanism plane from day one. The rest of v1.x and beyond (MCP-as-REST, Git-connect builds) stay in the architecture doc §12; don't plan them yet.
