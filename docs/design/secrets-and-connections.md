...
CREATE TABLE app_secrets (
  ...
  material     TEXT NOT NULL,            -- dev: iv:tag:ciphertext ; prod: Key Vault reference (URI), never plaintext
...
- **The policy edge (`helix_edge`) never reads `material`.** It is *not* granted SELECT on `app_secrets.material` (it is granted SELECT on the *metadata* it needs to authorize — see below), and in prod it has no Key Vault access at all. The role-split integration test (`role-split.integration.test.ts`) grows a case asserting `helix_edge` cannot read the `material` column, the same way it asserts the edge can't enumerate collections.
...
| Compromised *policy edge* dumps all keys | `helix_edge` has no SELECT on `material`, no local-envelope key, and no Key Vault access; only `helix_egress` resolves plaintext (§4). Extraction to an egress container makes this physical. |
...
4. **Key Vault `SecretStore` impl (the prod store) + the egress-container extraction.** The vault-as-store custody of §3 — required for any real deployment, not optional hardening — plus moving `helix_egress` out of the policy process, which makes §1's invariant physical. Tied to the architecture §3 egress-isolation milestone and the custom-backends §11 "third real app" line.
