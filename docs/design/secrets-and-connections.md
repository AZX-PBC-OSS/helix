...
4. **Key Vault `SecretStore` impl (the prod store).** The vault-as-store custody of §3 — required for any real deployment, swapped in behind the seam with no caller changes. Tied to the M5 Azure milestone and the custom-backends §11 "third real app" line. (The egress *container* itself is not deferred here — it ships in rung 1.)
