...
- **Per-IP rate limiting + size caps** on `collection` appends (the anonymous writer has no per-user budget to charge). Item-size cap and total-collection-size cap to bound storage.
