...
- ~~**Shared-write conflict semantics** (last-write-wins vs optimistic concurrency on `updatedAt`) — only matters once `sharedWrite` has a real user; default LWW.~~ **Answered by [ADR-0041](../adr/0041-app-data-write-concurrency.md):** optimistic concurrency, but on an opaque monotonic `version` column rather than `updatedAt` (which cannot round-trip through node-postgres without losing precision), and **mandatory** on `shared` — a `PUT` with no precondition is refused — while `user` scope keeps LWW by default.
