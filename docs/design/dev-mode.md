...
**Status:** Design draft **v2** · July 2026 (reconciled against the M4/M4.5 hardening now landed locally; `ownsApp`/issue #9 has since landed in `dc2aacf` — the BOLA prerequisite is closed, leaving domain-split/#16 as the one remaining, soft prerequisite)
...
- **dev-gateway placement:** ~~open~~ **now coupled to ADR-0019 / issue #16.** The Pre-GA move is to host untrusted apps on a *separate registrable domain* (`*.azx-apps.<tld>`), keeping the control plane on `azx.helix.azxlabs.io`. The dev-gateway is neither an app subdomain nor the app domain — it's a control-plane-adjacent surface — so it belongs on the control-plane eTLD+1 (e.g. `dev-api.azx.helix.azxlabs.io`), and its host name should be **decided in the same pass as the domain split**, not before it. A dedicated host still wins (clean CORS config + `helix_dev` isolation); the new constraint is only *which* eTLD+1 it sits under. Also make it a **per-plane opt-in flag** in the mold of `EDGE_ALLOW_PUBLIC_APPS` / `EDGE_ALLOW_PASSWORD_APPS` (`663bf1f`) so the dev surface can be disabled per environment.
...
- **Domain split / ADR-0019 (issue #16) — ⏳ still open (Pre-GA).** Decide the dev-gateway host in the same pass (§10). Doesn't block the schema/role work (step 1) or the dev-token work (step 2); only the dev-gateway surface (step 3) shouldn't pick a hostname before it. This is the one remaining named prerequisite, and it is soft — it gates a hostname choice, not the isolation work.
