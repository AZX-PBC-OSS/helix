...
- **Rate limiting / throttle is a shared Postgres counter now**, not in-memory (ADR-0011, `b939318`, `rate_counters` table). This matters because the dev-gateway is a *new replica surface*: its per-env budgets (§5.1) and any dev-side throttle ride the same shared counter by construction. The `EDGE_TRUST_PROXY` residual (issue #13) is now closed — the hop count is verified and the deployments pass `edgeTrustProxy` to the dev-gateway as well as the edge, so its `${req.ip}:${appId}` throttle key resolves to the real client behind ingress.
