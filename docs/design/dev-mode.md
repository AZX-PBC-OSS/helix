...
- **Rate limiting / throttle is a shared Postgres counter now**, not in-memory (ADR-0011, `b939318`, `rate_counters` table). This matters because the dev-gateway is a *new replica surface*: its per-env budgets (§5.1) and any dev-side throttle ride the same shared counter by construction. The `EDGE_TRUST_PROXY` residual (issue #13) is **closed as of 2026-09-03** — the deployments pass `edgeTrustProxy` (the trusted ingress **address**) to the dev-gateway as well as the edge, so its `${req.ip}:${appId}` throttle key resolves to the real client behind ingress. Note what the verification found: the default named the ACA *apps* subnet, which the ingress peer is not in; it is `100.64.0.0/10` (RFC 6598) now (ADR-0011's 2026-09 amendment).
...
The short-window **rate limiting / throttle** is a separate mechanism from the daily budget, and since v1 it moved to a shared Postgres counter (`rate_counters`, ADR-0011 / `b939318`) precisely so it holds across replicas. The dev-gateway is a *new* replica surface, so this is load-bearing: its throttle rides the same shared counter (never a per-process in-memory count), keyed like the edge's on `${req.ip}:${appId}`. The `EDGE_TRUST_PROXY` residual (issue #13) that this inherited is closed (2026-09-03): the deployments pass `edgeTrustProxy` — the trusted ingress **address**, not a hop count, which fastify 5.12.1 removed (GHSA-3m5p-2c4r-xxw2) — to the dev-gateway container as well as the edge, so its per-client limits resolve the real client IP. The verification found the shipped default wrong (it named the apps subnet, not the RFC 6598 range ACA's ingress pods come from); it is `100.64.0.0/10` now (ADR-0011's 2026-09 amendment). The dev-gateway's own **short-window throttle** is still the open piece — see the riders below.
