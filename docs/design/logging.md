...
| `auth.oidc_missing_oid` | `apps/edge/src/auth/oidc.ts` — the login refused because the ID token lacks the canonical principal id (ADR-0048 decision 2; no fallback exists, so this line is the whole diagnosis) |
| `auth.token_missing_oid` | `apps/portal/src/auth/verifier.ts` — the access-token mirror of the same refusal: a correctly signed token with no `oid` claim yields no actor |
