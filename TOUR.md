...
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **`apps/edge`** — data/policy plane | Faces untrusted app users (eventually anonymous internet). Terminates TLS, auth, serving, and the `/_api/*` gateway _policy_ (authz, quota, audit). | Read a registry projection; append metering rows; serve assets. (Holds its own operational keys — auth/instruction/OIDC secrets — and reads Blob under a read-only managed identity.) | Read an **app connection secret** (no grant on `app_secrets`). Write the registry. Make an **arbitrary** outbound call (only _through_ egress). |
...
| **`apps/dev-idp`** — _dev only_     | A local OIDC issuer (fixture users alice/bob/mallory/dana, stable `oid`s and pairwise `sub`s like Entra) standing in for Entra ID. Never deployed.  | Mint tokens for local dev/test.                                                                                                                                                       | Exist in production.                                                                                                                            |
...
holds its own operational keys, and it reads Blob under a **read-only managed identity**, the
over-broad account key ADR-0001 flagged having been removed in
[ADR-0027](docs/adr/0027-blob-auth-managed-identity.md) — but those are not the third-party
...
| Explain the platform to someone outside the team | [`docs/OVERVIEW.md`](docs/OVERVIEW.md) — problem, architecture, and security model in one file              |
| Find your way around the docs themselves         | [`docs/README.md`](docs/README.md) — what lives in which directory, and why                                 |
