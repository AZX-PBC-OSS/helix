...
- [ ] **Put RLS on `sessions` (deferred from the item above).** `sessions` has full DML and no RLS. Every query is already app-scoped **except** `PgSessionStore.sweep()`, which is deliberately global (GC across all apps, no `appId` in scope) — so a naive `app.app_id` partition policy would make the sweeper delete zero rows. Needs a sweeper carve-out (e.g. a separate policy / a distinct maintenance path) rather than the straight `withPartition` treatment the other two tables got. — ADR-0002 (ISSUE-12)
