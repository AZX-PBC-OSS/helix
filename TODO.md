...
- [ ] **Make egress's two dev seams refuse to start in production.** `EGRESS_ALLOW_INSECURE_CONNECTION` (permits injecting a connection secret into a cleartext `http://` target — the control added for issue #11) and `EGRESS_ALLOW_PRIVATE` (permits private/loopback/IMDS targets — the app-level half of the SSRF denylist) are both commented "false in prod" in `apps/egress/src/config.ts:20-29`, but nothing enforces it: `loadConfig` reads each as a plain `=== "true"` and starts. Every sibling dev seam on the other two planes **throws** instead — `EDGE_DEV_ALLOW_UNAUTHENTICATED` (`apps/edge/src/config.ts:610`), `PORTAL_ALLOW_SELF_APPROVE` (`apps/portal/src/plugins/auth.ts:90`), and the dev-token verifier, which refuses to construct. So this is an inconsistency in an established pattern, not a missing design. **The live deploys are safe** — neither var is set (confirmed 2026-08-06) — which is exactly why it is worth closing now rather than after an incident: each one silently disables a control ADR [0005](docs/adr/0005-ssrf-egress-controls.md) rests on, on the one plane that holds plaintext credentials, and with either set everything still works. Three lines in `loadConfig`, mirroring the edge's wording. — ADR-0005, found reconciling `docs/auth-review-guide.md` (2026-08-06)
